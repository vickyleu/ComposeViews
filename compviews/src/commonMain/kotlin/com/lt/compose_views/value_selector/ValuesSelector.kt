        ...
        Row {
            repeat(valuesList.size) {
                ValueSelector(
                    ...
                    defaultSelectIndex = remember(
                        defaultSelectIndexList,
                        it
                    ) { defaultSelectIndexList[it] },
                    isLoop = isLoop,
                )
            }
        }
...
internal fun BoxScope.CenterLines() {
    Column(Modifier.fillMaxWidth().align(Alignment.Center)) {
        HorizontalDivider(modifier = Modifier.fillMaxWidth().height(1.dp), color = lineColor)
        VerticalSpace(48)
        HorizontalDivider(modifier = Modifier.fillMaxWidth().height(1.dp), color = lineColor)
    }
}
